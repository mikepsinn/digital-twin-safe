...
export default function Info(props) {
  const { title, description, quantity, socialMediaUrl, locked, handleToggleLock, previewMode } = props
    ...
    <div className={classes.root}>
      <Grid container justify="space-between" className={classes.gridContainer}>
        ...
        <Grid item className={classes.rightAlignedText}>
          ...
          <div style={{ height: 8 }} />
          {locked ? (
            <Button id="unlockButton" onClick={handleToggleLock} variant="contained" startIcon={<LockOpenIcon />}>
              Unlock
            </Button>
          ) : (
            <Button onClick={handleToggleLock} variant="contained" startIcon={<LockIcon />}>
              Lock
            </Button>
          )}
        </Grid>
