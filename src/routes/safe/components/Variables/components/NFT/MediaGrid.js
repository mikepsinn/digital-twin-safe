...
export default function MediaGrid(props) {
  ...
  const playerTagForFile = (file) => {
    ...
    if (file.type.includes('image')) {
      return <img className={classes.imageAndVideo} src={fileUrl} style={{ maxHeight: maxHeight || '100%' }} />
  ...
  return (
    <div className={classes.root} style={{ height: '100%' }}>
      <Grid container spacing={1} justify="center" className={classes.fullHeight} style={{ height: '100%' }}>
        {jsx}
      </Grid>
    </div>
  )
