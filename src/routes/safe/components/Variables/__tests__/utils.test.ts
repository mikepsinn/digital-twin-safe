import { getVariableIcon, isVariableManifestValid } from '../utils'

describe('UserVariable manifest', () => {
  it('It should return true given a manifest with mandatory values supplied', () => {
    ...
    const result = isVariableManifestValid(manifest)
  ...
  it('It should return the best icon given an icons array', () => {
    ...
    expect(getVariableIcon(icons)).toBe('five.svg')
    icons.splice(icons.length - 1, 1)
    expect(getVariableIcon(icons)).toBe('three.png')
    icons.splice(icons.length - 2, 1)
    expect(getVariableIcon(icons)).toBe('four.png')
    icons.splice(icons.length - 1, 1)
    expect(getVariableIcon(icons)).toBe('one.png')
  })
})
